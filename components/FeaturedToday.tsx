    ...
    <>
      ...
      <div className="toppickscontainer w-full m-5 flex flex-row gap-2 overflow-x-scroll">
        {data.map((d: any) => (
          <>
            {d.poster_path ?
              <div key={d.title} className="showscontainer  bg-[#1A1A1A] w-1/3 h-fit pb-2 lg:1/5 rounded-b-md ">
                ...
                <Link key={d.id} href={`/posts?postId=${d.id}`}>
                  ...
                  <h2 className="mx-2 hover:underline cursor-pointer font-bold">{d.title}</h2>
                  </div>
                </Link>
                ...
                <div className="trailercontainer flex items-center justify-between w-full gap-5 my-1">
                  ...
                  <div className="infocontainer mx-10 hover:bg-white p-1 hover:bg-opacity-10 rounded-full cursor-pointer hidden lg:block">
